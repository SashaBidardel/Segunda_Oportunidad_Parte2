...
def query_productos_usados(sf, year):
    try:
        query = f"""
            SELECT Id, Name, Precio_Compra__c, Precio_Venta__c, Segunda_Oportunidad__c, Comprador__c
...
def query_segunda_oportunidad(sf, segunda_oportunidad_id):
    try:
        query = f"""
        ...
        print(f"Error al ejecutar consulta SOQL para Segunda Oportunidad: {e}")
        return None
