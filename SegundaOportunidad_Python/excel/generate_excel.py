...
from data_base.operations import connec_mysql
...
def generar_excel(conn):
    try:
        with conn.cursor() as cursor:
            ...
            sql_select = """
                SELECT dni_hash, nombre, email, beneficios_anio, cantidad_a_pagar,  total_compras, impuesto_transmisiones
                FROM usuarios
            """
            ...
            df = pd.DataFrame(result, columns=['DNI Hash', 'Nombre', 'Email', 'Beneficios Anio', 'Cantidad a Pagar', 'Total Compras', 'Impuesto Transmisiones'])
